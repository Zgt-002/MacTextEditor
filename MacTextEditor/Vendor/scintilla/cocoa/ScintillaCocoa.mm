...
void ScintillaCocoa::NotifyParent(NotificationData scn) {
	UpdateCharacterPositionCache(scn);
	...
	if (scn.nmhdr.code == Notification::UpdateUI) {
		NSView *content = ContentView();
		if ([content isAccessibilityElement] && FlagSet(scn.updated, Update::Content)) {
			NSAccessibilityPostNotification(content, NSAccessibilityValueChangedNotification);
		}
		if ([content isAccessibilityElement] && FlagSet(scn.updated, Update::Selection)) {
...
NSRange ScintillaCocoa::PositionsFromCharacters(NSRange rangeCharacters) const {
	EnsureCharacterPositionCache();
	Sci::Position basePosition = lastCharacterPosition;
	Sci::Position baseCharacter = lastCharacterIndex;
	Sci::Position bestDistance = std::abs(baseCharacter - static_cast<Sci::Position>(rangeCharacters.location));
	for (const auto &[position, character] : characterPositionCheckpoints) {
		const Sci::Position distance = std::abs(character - static_cast<Sci::Position>(rangeCharacters.location));
	...
	Sci::Position resolvedCharacter = static_cast<Sci::Position>(rangeCharacters.location);
	Sci::Position start = pdoc->GetRelativePositionUTF16(
		basePosition,
		resolvedCharacter - baseCharacter);
	if (start == Sci::invalidPosition)
		start = pdoc->GetRelativePositionUTF16(0, rangeCharacters.location);
	if (start == Sci::invalidPosition) {
		start = pdoc->Length();
		resolvedCharacter = CharactersFromPositions(NSMakeRange(start, 0)).location;
	}
	Sci::Position end = pdoc->GetRelativePositionUTF16(start, rangeCharacters.length);
	if (end == Sci::invalidPosition)
		end = pdoc->Length();
	lastCharacterPosition = start;
	lastCharacterIndex = resolvedCharacter;
...
NSRange ScintillaCocoa::CharactersFromPositions(NSRange rangePositions) const {
	EnsureCharacterPositionCache();
	const Sci::Position target = pdoc->ClampPositionIntoDocument(rangePositions.location);
	Sci::Position basePosition = lastCharacterPosition;
	Sci::Position baseCharacter = lastCharacterIndex;
	Sci::Position bestDistance = std::abs(basePosition - target);
	for (const auto &[position, character] : characterPositionCheckpoints) {
		const Sci::Position distance = std::abs(position - target);
		if (distance < bestDistance) {
			basePosition = position;
			baseCharacter = character;
			bestDistance = distance;
	...
	const Sci::Position start = target >= basePosition
		? baseCharacter + pdoc->CountUTF16(basePosition, target)
		: baseCharacter - pdoc->CountUTF16(target, basePosition);
	const Sci::Position endPosition = pdoc->ClampPositionIntoDocument(NSMaxRange(rangePositions));
	const Sci::Position len = pdoc->CountUTF16(target, endPosition);
	if (target == sel.MainCaret() || target == sel.MainAnchor()) {
...
void ScintillaCocoa::ResetCharacterPositionCache() const {
	characterPositionDocument = pdoc;
	characterPositionCheckpoints = {{0, 0}};
	lastCharacterPosition = 0;
	lastCharacterIndex = 0;
}

void ScintillaCocoa::EnsureCharacterPositionCache() const {
	if (characterPositionDocument != pdoc || characterPositionCheckpoints.empty() ||
		lastCharacterPosition < 0 || lastCharacterPosition > pdoc->Length()) {
		ResetCharacterPositionCache();
	}
}

void ScintillaCocoa::UpdateCharacterPositionCache(const NotificationData &notification) {
	if (notification.nmhdr.code != Notification::Modified ||
		!FlagSet(notification.modificationType,
			ModificationFlags::InsertText | ModificationFlags::DeleteText)) {
		return;
	}
	EnsureCharacterPositionCache();
	if (!notification.text || notification.length <= 0) {
		ResetCharacterPositionCache();
		return;
	}

	const Sci::Position position = notification.position;
	const Sci::Position length = notification.length;
	const Sci::Position characterLength = static_cast<Sci::Position>(
		UTF16Length(std::string_view(notification.text, static_cast<size_t>(length))));
	if (FlagSet(notification.modificationType, ModificationFlags::InsertText)) {
		for (auto &[checkpointPosition, checkpointCharacter] : characterPositionCheckpoints) {
			if (checkpointPosition > position) {
				checkpointPosition += length;
				checkpointCharacter += characterLength;
			}
		}
		if (lastCharacterPosition > position) {
			lastCharacterPosition += length;
			lastCharacterIndex += characterLength;
		}
		if (length >= 1024 * 1024) {
			const Sci::Position characterAtStart = CharactersFromPositions(
				NSMakeRange(position, 0)).location;
			characterPositionCheckpoints.emplace_back(
				position + length,
				characterAtStart + characterLength);
			std::sort(characterPositionCheckpoints.begin(), characterPositionCheckpoints.end());
		}
	} else {
		const Sci::Position oldEnd = position + length;
		characterPositionCheckpoints.erase(
			std::remove_if(
				characterPositionCheckpoints.begin(),
				characterPositionCheckpoints.end(),
				[position, oldEnd](const auto &checkpoint) {
					return checkpoint.first > position && checkpoint.first < oldEnd;
				}),
			characterPositionCheckpoints.end());
		for (auto &[checkpointPosition, checkpointCharacter] : characterPositionCheckpoints) {
			if (checkpointPosition >= oldEnd) {
				checkpointPosition -= length;
				checkpointCharacter -= characterLength;
			}
		}
		if (lastCharacterPosition >= oldEnd) {
			lastCharacterPosition -= length;
			lastCharacterIndex -= characterLength;
		} else if (lastCharacterPosition > position) {
			ResetCharacterPositionCache();
		}
	}
}
